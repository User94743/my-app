...
const TEMP_PARAMS = 'TEMP-PARAMS';

export const updateParameter = (value, machineId, param) => {
...
export const getNumMachine = (numberMachine) => {
 return {
  type: TEMP_PARAMS,
  numberMachine
 }
...
const parametersReducer = (state = initialState, action) => {
 ...
 switch(action.type) {
  case TEMP_PARAMS:
   return {
    ...state, currentMachine: state.machine.filter(m => m.number === action.numberMachine)[0]
   }
